    ...
    where TVpnManager : IVpnService<TClientConnection>
{
    ...
    {
        if (TryGetConnection(id, out var connection) == false || connection is null)
    ...
    public ConnectionDto CreateConnection(CreateConnectionDto dto)
    {
        ...
        var info = VpnManager.GenerateConnectionInfo();
    ...
    private bool TryGetConnection(int id, out Connection? connection)
    {
        connection = null;
        if (id < 0)
            throw new InvalidArgumentException(
